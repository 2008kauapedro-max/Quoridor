/* =============================================================
   ...
   -------------------------------------------------------------
   Lógica PURA da partida (zero DOM). Usada por:
   local · online (host e guest validam igual) · IA · replay.
   ============================================================= */
import { SIZE, WALLS_PER_PLAYER, GOAL } from "./constants.js";
...
export function canMoveTo(state, r, c, nr, nc){
  ...
  if (Math.abs(dr) + Math.abs(dc) !== 1) return false;          // 1 casa ortogonal
  if (dr === -1 && state.hSeg[r - 1][c]) return false;          // parede acima
  if (dr ===  1 && state.hSeg[r][c])     return false;          // parede abaixo
  if (dc === -1 && state.vSeg[r][c - 1]) return false;          // parede esquerda
  if (dc ===  1 && state.vSeg[r][c])     return false;          // parede direita
...
export function legalMoves(state, player){
  ...
  for (const [dr, dc] of [[-1,0],[1,0],[0,-1],[0,1]]){
    ...
    // canMoveTo checa "turn"; aqui queremos para QUALQUER player:
...
export function applyMove(state, r, c){
  ...
  if (!canMoveTo(state, p.r, p.c, r, c)) return null;   // anti clique-duplo/bug
  p.r = r; p.c = c;
...
export function validateWall(state, type, r, c){
  ...
  setWallSegments(state, type, r, c, true);              // simula
  const okRed  = findPathBFS(state, state.players.red.r,  state.players.red.c,  GOAL.red);
  const okBlue = findPathBFS(state, state.players.blue.r, state.players.blue.c, GOAL.blue);
  setWallSegments(state, type, r, c, false);             // desfaz
...
export function bfsDistance(state, startR, startC, goalRow){
  ...
  while (head < queue.length){
    ...
    if (r < SIZE - 1 && !state.hSeg[r][c]     && !seen[r + 1][c]){ seen[r+1][c]=true; queue.push([r+1,c,d+1]); }
    if (c > 0        && !state.vSeg[r][c - 1] && !seen[r][c - 1]){ seen[r][c-1]=true; queue.push([r,c-1,d+1]); }
    if (c < SIZE - 1 && !state.vSeg[r][c]     && !seen[r][c + 1]){ seen[r][c+1]=true; queue.push([r,c+1,d+1]); }
  }
  return -1;
}
...
export function checkVictory(state){
  if (state.players.red.r  === GOAL.red){  state.over = true; state.winner = "red";  }
  if (state.players.blue.r === GOAL.blue){ state.over = true; state.winner = "blue"; }
  return state.over;
}

/* Marca se o jogador estava ATRÁS em algum momento (conquista virada) */
function trackBehind(state){
  const redDist  = state.players.red.r;                 // linhas p/ o topo
  const blueDist = (SIZE - 1) - state.players.blue.r;   // linhas p/ a base
