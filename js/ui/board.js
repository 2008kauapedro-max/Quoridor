...
export function pieceBgFor(id, color, online = false){
  const it = SKIN_CATALOG.find((i) => i.cat === "piece" && i.id === id);
  /* skin de time: no local/IA só o VERMELHO usa o time (azul fica clássico);
     no online, QUEM equipou usa o time na sua cor */
  if (it && it.badge && (online || color === "red")) return it.badge;
  const sw = pieceSwatch(id);
